...
const ExamConduiteTable = () => {
  ...
  const removeData = (id) => {
    ...
    axios.delete(`${URL}/${id}`).then(res => {
      const del = exams.filter(employee => id !== employee.id)
      setexams(del)
    })
  }
  ...
  const renderBody = () => {
    return exams && exams.map(({ id, type, date, startTime, monitorCin, userCin, plaque, duration }) => {
      if (type === 2)
        return (
          <tr key={id}>
            <td>{monitorCin}</td>
  ...
  return (
    <div>
      <br />
      <h1 id='title' className="seanceconduitetitle">Driving Exams Table</h1>
      <div className="add">
        <Popup
        contentStyle={{ width: "500px" }} 
        trigger={<button className="btn-custom-sc1 btn-custom-sc18 "><AddIcon /> New </button>} position=" top right">
        <div>
          <div style={{
            width: '100%',
      ...
      <br />
      <table id='students' className="seanceconduite">
        <thead>
          <tr>{renderHeader()}</tr>
        </thead>
        <tbody>
...
export default ExamConduiteTable