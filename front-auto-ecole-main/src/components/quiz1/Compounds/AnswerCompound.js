...
function AnswerCompound({ children, setStep }) {
  ...
  const [showAnswerResult, setShowAnswerResult] =
    useContext(AnswerResultContext);
  const [correctAnswersNumber, setCorrectAnswersNumber] = useContext(
    CorrectAnswersContext
  );
  const [wrongAnswersNumber, setWrongAnswersNumber] =
    useContext(WrongAnswersContext);
  ...
  return (
    <div>
      <div className="rowquiz">
        <div className="colquiz">
          {" "}
          <img
            className="image"
            alt="world"
            src={QuestionsData[currentQuestion - 1].source}
          >
          </img>
        </div>
        <div className="colquiz" id="answers">
